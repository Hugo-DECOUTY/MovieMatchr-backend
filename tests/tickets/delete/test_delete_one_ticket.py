...
@pytest.mark.asyncio
async def test_delete_one_ticket_from_admin(client: AsyncClient, session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_delete_one_ticket_from_local_admin(client: AsyncClient, session: AsyncSession) -> None:
    await pre_fill_db(session)
    response = await client.get(
            "/tickets",
