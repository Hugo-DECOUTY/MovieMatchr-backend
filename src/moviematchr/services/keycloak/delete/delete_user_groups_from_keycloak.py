...
async def delete_user_groups_from_keycloak(user_id: str, group_id: int):
    ...
    if group_id == TypeLicenceEnum.DOCTOR.value:
        async with httpx.AsyncClient() as client_async:
            response = await client_async.delete(
                f"{REALM_URL}/users/{user_id}/groups/{DOCTOR_GROUP_ID}",
                headers={"Authorization": f"Bearer {token['access_token']}"},
            )

            if response.status_code != 204:
                raise HTTPException(response.status_code, detail="User not updated in keycloak (DOCTOR)")
        ...
        async with httpx.AsyncClient() as client_async:
            ...
            if response.status_code != 204:
                raise HTTPException(response.status_code, detail="User not updated in keycloak (MEDICAL STAFF)")
        ...
        async with httpx.AsyncClient() as client_async:
            ...
            if response.status_code != 204:
                raise HTTPException(response.status_code, detail="User not updated in keycloak (NON MEDICAL STAFF)")
        ...
        async with httpx.AsyncClient() as client_async:
            ...
            if response.status_code != 204:
                raise HTTPException(response.status_code, detail="User not updated in keycloak (HDS LOCAL ADMIN)")
    else:
