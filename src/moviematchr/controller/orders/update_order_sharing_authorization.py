from fastapi import HTTPException, Request
from sqlalchemy.ext.asyncio import AsyncSession

from moviematchr.enum.errors.HttpErrorsEnum import HttpErrorsEnum

from moviematchr.schemas.orders.orders import Orders

from moviematchr.services.orders import get_order_dal, update_order_dal

from moviematchr.utils.socket import socket_manager as sio
from moviematchr.utils.user_group import UserGroup, get_payload_and_groups


async def update_order_sharing_authorization(
    db: AsyncSession, request: Request, order_id: str, sharing_authorization: bool
) -> dict:
    payload, group = get_payload_and_groups(request.headers.get("X-USERINFO"))
    if group == UserGroup.HDS_ADMIN_MICROPORT.value:

        order: Orders = await get_order_dal(db, order_id)

        if order is None:
            raise HTTPException(
                status_code=404, detail=HttpErrorsEnum.ORDER_NOT_FOUND.value
            )

        order.sharing_authorization = sharing_authorization

        await update_order_dal(db, order)
        await sio.emit("orders", room=payload["sub"])
        return {"status_code:": 204, "message": "Order sharing authorization updated"}

    raise HTTPException(status_code=403)
